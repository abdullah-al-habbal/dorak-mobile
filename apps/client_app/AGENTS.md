...
lib/src/core/navigation/
  ...
  app_gate.entity.dart         AppGate.decide — the post-splash branch
lib/src/features/
...
`AppGate.decide` — two independent checks, strict order, installed as the
router's redirect:
...
plus `widgets/{auth_header, auth_text_field, otp_input_field, auth_error_banner,
login_content, sign_up_content, verify_account_content}.widget.dart` and
`auth_validators.entity.dart` / `auth_error.entity.dart`.
...
| `AuthHeader`'s trailing `SizedBox(width: 48)` | Balances the leading `IconButton` so the brand stays centred. |
